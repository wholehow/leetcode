...
public class Solution53 {
	...
	/* 这题是一道经典的dp问题，我们可以很容易的得到其dp方程，假设dp[i]是数组nums[0, i]区间最大的值，那么
	...
	private int divide(int[] nums, int left, int right, int tmpMax) {
		if (left > right) {
			return Integer.MIN_VALUE;
		}
		int mid = (left + right) / 2;
		//1.最大值在nums[left, mid - 1]里面
		int lmax = divide(nums, left, mid, tmpMax);  //得到子区间[left, mid - 1]最大值
		//2.最大值在nums[mid + 1, right]里面
		int rmax = divide(nums, mid + 1, right, tmpMax);  //得到子区间[mid + 1, right]最大值
