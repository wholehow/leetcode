...
public class Solution414 {
	 ...
	 * 我们以数组中第一个数字来作为初始值，
	 * 如果当前数字比第一个数字大，那么原本第一个数字的值要移动到第二个，原本第二个数字的值要移动到第三个；
	 * 如果替代的是第二个数字，同样要把原本第二个数字的值移动到第三个。
	 * 由于我们是用第一个数字作为初始值的，因此在替换数字时还有一个原因就是如果第二个和第三个数字还是初始值，
	...
	public int thirdMax(int[] nums) {
        ...
        for (int i = 1; i < nums.length; i++) {
            if (nums[i] > firstMax) {
                ...
                secondMax = firstMax;
                firstMax = nums[i];
            } else if (nums[i] != firstMax && (nums[i] > secondMax || secondMax == firstMax)) {
                thirdMax = secondMax;
                secondMax = nums[i];
            } else if ((nums[i] != firstMax && nums[i] != secondMax) && (nums[i] > thirdMax || thirdMax == secondMax || thirdMax == firstMax)) {
                thirdMax = nums[i];
            }
        }
